...
/**
 * Provides application settings and controls to its descendant components via SettingsContext.
 *
 * The provider manages persisted settings (admin PIN, theme, board interval, ticker duration),
 * the current role and a requestAdminMode flow, and a resetData/registerResetData pair for
 * registering and invoking a reset callback.
 *
 * @param children - React nodes rendered inside the provider
 * @returns A React element that supplies settings state and control functions through SettingsContext
 */
...
/**
 * Retrieve the current settings context.
 *
 * @returns The settings context value containing role, adminPin, theme, boardInterval, tickerDuration and their updater functions, plus resetData helpers.
 * @throws Error if called outside of a SettingsProvider (message: 'useSettings must be used within SettingsProvider').
 */
